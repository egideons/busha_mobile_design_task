import 'xtz_tx_block_model.dart';

class XtzBlockResponseModel {
  List<XtzTxBlockModel> blocks;

  XtzBlockResponseModel({
    required this.blocks,
  });

  factory XtzBlockResponseModel.fromJson(List<dynamic>? parsedJson) {
    parsedJson ??= <dynamic>[];
    List<XtzTxBlockModel> blocks =
        parsedJson.map((i) => XtzTxBlockModel.fromJson(i)).toList();
    return XtzBlockResponseModel(blocks: blocks);
  }
...
//   factory XtzBlockResponseModel.fromJson(Map<String, dynamic>? json) {
//     json = {};
//     return XtzBlockResponseModel(
//       cycle: json['cycle'] ?? 0,
//       level: json['level'] ?? 0,
//       hash: json['hash'] ?? "",
